// test.js - Simple test file for LaTeX converter
const LATEX_TO_UNICODE_MAP = {
    // Acute accents (´) - Multiple syntax variations
    "{\\'a}": "á", "{\\'A}": "Á",
    "{\\'e}": "é", "{\\'E}": "É",
    "{\\'i}": "í", "{\\'I}": "Í",
    "{\\'o}": "ó", "{\\'O}": "Ó",
    "{\\'u}": "ú", "{\\'U}": "Ú",
    "{\\'y}": "ý", "{\\'Y}": "Ý",
    "{\\'c}": "ć", "{\\'C}": "Ć",
    "{\\'n}": "ń", "{\\'N}": "Ń",
    "{\\'s}": "ś", "{\\'S}": "Ś",
    "{\\'z}": "ź", "{\\'Z}": "Ź",
    // Alternative syntax variations
    "\\'{a}": "á", "\\'{A}": "Á",
    "\\'{e}": "é", "\\'{E}": "É",
    "\\'e": "é", "\\'E": "É",
    "\\'a": "á", "\\'A": "Á",
    "\\'{i}": "í", "\\'{I}": "Í",
    "\\'{o}": "ó", "\\'{O}": "Ó",
    "\\'{u}": "ú", "\\'{U}": "Ú",

    // Grave accents (`)
    "{\\`a}": "à", "{\\`A}": "À",
    "{\\`e}": "è", "{\\`E}": "È",
    "{\\`i}": "ì", "{\\`I}": "Ì",
    "{\\`o}": "ò", "{\\`O}": "Ò",
    "{\\`u}": "ù", "{\\`U}": "Ù",

    // Circumflex (^)
    "{\\^a}": "â", "{\\^A}": "Â",
    "{\\^e}": "ê", "{\\^E}": "Ê",
    "{\\^i}": "î", "{\\^I}": "Î",
    "{\\^o}": "ô", "{\\^O}": "Ô",
    "{\\^u}": "û", "{\\^U}": "Û",

    // Umlaut/Diaeresis (¨) - Multiple syntax variations
    "{\\\"a}": "ä", "{\\\"A}": "Ä",
    "{\\\"e}": "ë", "{\\\"E}": "Ë",
    "{\\\"i}": "ï", "{\\\"I}": "Ï",
    "{\\\"o}": "ö", "{\\\"O}": "Ö",
    "{\\\"u}": "ü", "{\\\"U}": "Ü",
    // Alternative syntax - braces around letter  
    '\\"{a}': "ä", '\\"{A}': "Ä",
    '\\"{e}': "ë", '\\"{E}': "Ë",
    '\\"{i}': "ï", '\\"{I}': "Ï",
    '\\"{o}': "ö", '\\"{O}': "Ö",
    '\\"{u}': "ü", '\\"{U}': "Ü",
    // Minimal syntax
    '\\"a': "ä", '\\"A': "Ä",
    '\\"e': "ë", '\\"E': "Ë",
    '\\"i': "ï", '\\"I': "Ï",
    '\\"o': "ö", '\\"O': "Ö",
    '\\"u': "ü", '\\"U': "Ü",

    // Tilde (~)
    "{\\~a}": "ã", "{\\~A}": "Ã",
    "{\\~e}": "ẽ", "{\\~E}": "Ẽ",
    "{\\~i}": "ĩ", "{\\~I}": "Ĩ",
    "{\\~o}": "õ", "{\\~O}": "Õ",
    "{\\~u}": "ũ", "{\\~U}": "Ũ",
    "{\\~n}": "ñ", "{\\~N}": "Ñ",

    // Caron/Háček (ˇ) - very common in Czech/Slovak
    "{\\v{a}}": "ǎ", "{\\v{A}}": "Ǎ",
    "{\\v{c}}": "č", "{\\v{C}}": "Č",
    "{\\v{d}}": "ď", "{\\v{D}}": "Ď",
    "{\\v{e}}": "ě", "{\\v{E}}": "Ě",
    "{\\v{g}}": "ǧ", "{\\v{G}}": "Ǧ",
    "{\\v{h}}": "ȟ", "{\\v{H}}": "Ȟ",
    "{\\v{i}}": "ǐ", "{\\v{I}}": "Ǐ",
    "{\\v{j}}": "ǰ", "{\\v{J}}": "J̌",
    "{\\v{k}}": "ǩ", "{\\v{K}}": "Ǩ",
    "{\\v{l}}": "ľ", "{\\v{L}}": "Ľ",
    "{\\v{n}}": "ň", "{\\v{N}}": "Ň",
    "{\\v{o}}": "ǒ", "{\\v{O}}": "Ǒ",
    "{\\v{r}}": "ř", "{\\v{R}}": "Ř",
    "{\\v{s}}": "š", "{\\v{S}}": "Š",
    "{\\v{t}}": "ť", "{\\v{T}}": "Ť",
    "{\\v{u}}": "ǔ", "{\\v{U}}": "Ǔ",
    "{\\v{z}}": "ž", "{\\v{Z}}": "Ž",

    // Cedilla (¸)
    "{\\c{c}}": "ç", "{\\c{C}}": "Ç",
    "{\\c{g}}": "ģ", "{\\c{G}}": "Ģ",
    "{\\c{k}}": "ķ", "{\\c{K}}": "Ķ",
    "{\\c{l}}": "ļ", "{\\c{L}}": "Ļ",
    "{\\c{n}}": "ņ", "{\\c{N}}": "Ņ",
    "{\\c{r}}": "ŗ", "{\\c{R}}": "Ŗ",
    "{\\c{s}}": "ş", "{\\c{S}}": "Ş",
    "{\\c{t}}": "ţ", "{\\c{T}}": "Ţ",

    // Ring above (˚)
    "{\\r{a}}": "å", "{\\r{A}}": "Å",
    "{\\r{u}}": "ů", "{\\r{U}}": "Ů",

    // Stroke/Bar
    "{\\l}": "ł", "{\\L}": "Ł",
    "{\\o}": "ø", "{\\O}": "Ø",
    "{\\d}": "đ", "{\\D}": "Đ",

    // Special characters
    "{\\ae}": "æ", "{\\AE}": "Æ",
    "{\\oe}": "œ", "{\\OE}": "Œ",
    "{\\ss}": "ß", "{\\SS}": "SS",

    // Common symbols
    "{\\&}": "&",
    "{\\$}": "$",
    "{\\%}": "%",
    "{\\#}": "#",
    "{\\_}": "_",
    "{\\{}": "{",
    "{\\}}": "}",
};
/**
 * @param {string} text - The input string containing LaTeX syntax.
 * @returns {string} - The converted string with LaTeX syntax replaced by Unicode characters
 */
export function convertLatexToUnicode(text) {
    if (!text) return text;

    let result = text;

    // Apply all mappings
    for (const [latex, unicode] of Object.entries(LATEX_TO_UNICODE_MAP)) {
        // Use global replacement with regex for compatibility
        const escapedLatex = latex.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        result = result.replace(new RegExp(escapedLatex, 'g'), unicode);
    }

    // Remove case-protection braces (but preserve content)
    // This handles {Driven}, {Development} etc.
    result = result.replace(/\{([^\\{}]+)\}/g, '$1');

    // Clean up any remaining double spaces
    result = result.replace(/\s+/g, ' ').trim();

    return result;
}

// Test with your examples
console.log("Testing conversion:");
console.log("Matja{\\v{z}} →", convertLatexToUnicode("Matja{\\v{z}}"));
console.log("platform {\\'e} →", convertLatexToUnicode("platform {\\'e}"));
console.log("Formalism-{Driven} {Development} →", convertLatexToUnicode("Formalism-{Driven} {Development}: {Concepts}, {Taxonomy}, and {Practice}"));

// Test multiple LaTeX syntax variations
console.log("\nTesting different syntax for ö:");
console.log('\\"{o} →', convertLatexToUnicode('\\"{o}'));
console.log('\\"o →', convertLatexToUnicode('\\"o'));

console.log("\nTesting different syntax for é:");
console.log("{\\'e} →", convertLatexToUnicode("{\\'e}"));
console.log("\\'{e} →", convertLatexToUnicode("\\'{e}"));
console.log("\\'e →", convertLatexToUnicode("\\'e"));